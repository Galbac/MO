...
@router.post("/news-categories", response_model=MessageResponse)
async def create_admin_news_category(payload: dict) -> MessageResponse:
    return await service.create_category(payload)


@router.patch("/news-categories/{category_id}", response_model=MessageResponse)
async def patch_admin_news_category(category_id: int, payload: dict) -> MessageResponse:
    return await service.update_category(category_id, payload)


@router.delete("/news-categories/{category_id}", response_model=MessageResponse)
async def delete_admin_news_category(category_id: int) -> MessageResponse:
...
@router.post("/tags", response_model=MessageResponse)
async def create_admin_tag(payload: dict) -> MessageResponse:
    return await service.create_tag(payload)


@router.patch("/tags/{tag_id}", response_model=MessageResponse)
async def patch_admin_tag(tag_id: int, payload: dict) -> MessageResponse:
    return await service.update_tag(tag_id, payload)


@router.delete("/tags/{tag_id}", response_model=MessageResponse)
async def delete_admin_tag(tag_id: int) -> MessageResponse:
