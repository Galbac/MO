...
class PortalQueryService:
    ...
    async def get_player_stats(self, player_id: int):
        async with db_session_manager.session() as session:
            matches = await self.players.get_matches(session, player_id)
            return SuccessResponse(data=self._player_stats(matches, player_id))
    ...
    async def get_player_titles(self, player_id: int):
        ...
        return SuccessResponse(data=payload.data.titles)
    ...
    async def get_player_upcoming_matches(self, player_id: int):
        payload = await self.get_player(player_id)
        return SuccessResponse(data=[payload.data.upcoming_match] if payload.data.upcoming_match else [])
    ...
    async def get_tournament(self, tournament_id: int):
        async with db_session_manager.session() as session:
            ...
            if final_match:
                champions.append(ChampionItem(season_year=tournament.season_year, player_name=players[final_match.winner_id].full_name))
            draw = [DrawMatchItem(round_code=match.round_code or '', player1_name=players[match.player1_id].full_name, player2_name=players[match.player2_id].full_name, score_summary=match.score_summary) for match in matches if players.get(match.player1_id) and players.get(match.player2_id)]
            detail = TournamentDetail(**self._tournament_summary(tournament).model_dump(), short_name=tournament.short_name, indoor=tournament.indoor, country_code=tournament.country_code or '', prize_money=tournament.prize_money, points_winner=tournament.points_winner, logo_url=tournament.logo_url, description=tournament.description, participants=participants, current_matches=current_matches, champions=champions, draw=draw, seo={'title': tournament.name, 'description': tournament.description or tournament.name})
            return SuccessResponse(data=detail)

    async def get_tournament_matches(self, tournament_id: int):
        payload = await self.get_tournament(tournament_id)
        return SuccessResponse(data=[MatchSummary.model_validate(item) for item in payload.data.current_matches])

    async def get_tournament_draw(self, tournament_id: int):
        payload = await self.get_tournament(tournament_id)
        return SuccessResponse(data=payload.data.draw)

    async def get_tournament_players(self, tournament_id: int):
        payload = await self.get_tournament(tournament_id)
        return SuccessResponse(data=[PlayerSummary.model_validate(item) for item in payload.data.participants])

    async def get_tournament_champions(self, tournament_id: int):
        payload = await self.get_tournament(tournament_id)
        return SuccessResponse(data=payload.data.champions)
