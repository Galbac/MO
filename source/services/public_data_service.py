...
from source.db.models import Match, MatchEvent, Player, RankingSnapshot, Tournament
...
class PublicDataService:
    ...
    async def get_race_rankings(self) -> SuccessResponse[list[RankingEntry]]:
        return await self.get_current_rankings('atp')
    ...
    async def search(self, q: str) -> SuccessResponse[SearchResults]:
        async with db_session_manager.session() as session:
            ...
            player_map = {item.id: item for item in await self.repo.list_players_by_ids(session, list({value for match in matches for value in (match.player1_id, match.player2_id)}))}
            match_payload = []
