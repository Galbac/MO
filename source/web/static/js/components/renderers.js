        ...
        <a class="entity-card fade-up" href="/players/${escapeHtml(player.slug)}">
            <div class="entity-card__head">
                <div class="d-flex gap-3">
                    ...
                    <div>
                        <div class="entity-card__eyebrow">Профиль игрока</div>
                        <h3 class="entity-card__title">${escapeHtml(player.full_name)}</h3>
                        <div class="entity-card__meta">Рейтинг №${escapeHtml(player.current_rank ?? "-")} • ${escapeHtml(player.country_name || player.country_code || "")}</div>
                    </div>
                </div>
            ...
            <div class="d-flex flex-wrap gap-2 mt-3">
                <span class="badge-soft">Очки ${escapeHtml(player.current_points ?? "-")}</span>
    ...
    return `
        <a class="entity-card fade-up" href="/tournaments/${escapeHtml(tournament.slug)}">
            <div class="entity-card__row">
                <div class="entity-card__eyebrow">${escapeHtml(statusLabel(tournament.category || "Турнир"))}</div>
...
export function matchCard(match) {
    ...
    return `
        <a class="entity-card fade-up" href="/matches/${escapeHtml(match.slug)}">
            <div class="entity-card__row">
                <div class="entity-card__eyebrow">${escapeHtml(match.tournament_name || "Центр матчей")}</div>
                <span class="${live ? "badge-live" : statusClass(match.status)}">${escapeHtml(statusLabel(match.status || "scheduled"))}</span>
            </div>
            <h3 class="entity-card__title">${escapeHtml(match.player1_name)} против ${escapeHtml(match.player2_name)}</h3>
            <div class="entity-card__meta">${escapeHtml(match.round_code || "Основная сетка")} • ${escapeHtml(formatDate(match.scheduled_at, { month: "short", day: "2-digit", hour: "2-digit", minute: "2-digit" }))}</div>
            <div class="d-flex justify-content-between align-items-center mt-3">
                <div class="badge-soft">ЛВ ${escapeHtml(match.h2h_summary || "Превью")}</div>
                <strong>${escapeHtml(match.score_summary || match.current_score || "Ожидание первой подачи")}</strong>
            </div>
        </a>`;
}
...
export function newsCard(article) {
    return `
        <a class="entity-card editorial-card fade-up" href="/news/${escapeHtml(article.slug)}">
            ...
            <div class="entity-card__eyebrow">${escapeHtml(article.category?.name || article.status || "Новость")}</div>
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                ...
                <span class="text-muted small">${escapeHtml(article.read_time || "3 мин чтения")}</span>
            </div>
        </a>`;
}
...
export function timelineItem(item) {
    ...
    return `
        <article class="timeline-item">
            <div class="timeline-item__time">${escapeHtml(item.event_type || item.type || "Событие")}</div>
            <strong>${escapeHtml(item.title || item.label || "Лайв-обновление")}</strong>
    ...
    return `
        <div class="scoreboard-card fade-up">
            <div class="d-flex flex-wrap justify-content-between gap-3 align-items-center mb-3">
                <div>
                    <div class="section-kicker">${escapeHtml(detail.tournament_name || "Лайв-центр")}</div>
                    <h1 class="section-title mt-2">${escapeHtml(detail.player1_name)} против ${escapeHtml(detail.player2_name)}</h1>
                    <div class="text-muted mt-1">${escapeHtml(detail.round_code || "Основная сетка")} • ${escapeHtml(formatDate(detail.scheduled_at))}</div>
                </div>
            ...
            <div class="scoreboard-card__grid">
                ...
                <div class="glass-card rounded-4 p-3 h-100">
                    <div class="section-kicker">Пульс матча</div>
                    <div class="metric-value mt-2">${escapeHtml(score.current_game || detail.score_summary || "До начала матча")}</div>
                    <div class="text-muted mt-2">Подает: ${escapeHtml(serveTarget || "Будет определено")}</div>
                    <div class="d-flex flex-wrap gap-2 mt-3">
                        <span class="badge-soft">Корт ${escapeHtml(detail.court_name || "-")}</span>
                        <span class="badge-soft">Рейтинг ${escapeHtml(detail.player1_rank || "-")} / ${escapeHtml(detail.player2_rank || "-")}</span>
                    </div>
                </div>
            </div>
        </div>`;
